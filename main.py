...
dev = qml.device('strawberryfields.fock', wires=qubit_number)
...
def cost(x, theta):
    ...
    for e, i in enumerate(x):
        x_in = kappa + np.tan(l*i)#np.arcsin(i) * 0.5
        ...
        ode = np.abs(np.array(F(df_x, f, x[e])))
        ## Add Mse
        loss += np.mean(np.array(ode ** 2))
        ## If x = 0 add regularization
        #if e == f_0_index:
        #    loss += np.abs(f - f_0) ** 2
        ## Add regularization term
        loss += np.mean(np.mean(np.abs(f - f_true(i)) ** 2))
    loss /= space_size
    print(loss._value)
